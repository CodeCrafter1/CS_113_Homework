...
public class MorseCodeTreeTest {
    ...
    /**
     * Every morse code value which represents a single letter from A-Z using
    ...
    /**
     * Entries containing a series of morse code values, each representing a
     * single letter value with dots ('*') and dashes ('-') and delimited from
     * each other using spaces.
     */
    ...
    /**
     * Erroneous input includes invalid characters (not '*' or '-'), or tokens
     * that are too long to represent any letter value.
     */
    ...
    /**
     * Your MorseCodeTree object containing the method
     * translateFromMorseCode(String).
     */
    ...
    /**
     * Helper method compares an array of morse code entries with a parallel
     * array of its corresponding English letter translation.
     *
     * @param morseCode The array of morse code values, using '*' for dots and
     * '-' for dashes and is delimited from each letter with a space.
     * @param decodedValue The array of English letter values expected after
     * translating the values in morseCode at the same indices.
     */
    private void compareDecodedValues(String[] morseCode, String[] decodedValue) {
        try {
            ...
            fail("Test failed - an Exception was thrown.");
        }
    }
